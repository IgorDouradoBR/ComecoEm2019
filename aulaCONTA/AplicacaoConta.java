...
public class AplicacaoConta
{
    public static void main(String args[]){
        ...
        c5.depositar(800);
        banco[0]=c1; // ou banco[0] = new Conta(1); ou banco[0].depositar(100); ou banco[0].sacar(5);
        banco[1]=c2;
        banco[2]=c3;
        banco[3]=c4;
        banco[4]=c5;
        ...
        while(true){
            int op = menu();//chamou o metodo menu
            if(op == 0) break;
            switch(op){
                ...
                for (int i=0;i<5;i++) 
                { 
                    System.out.println(banco[i]); // ou banco[i]toString();
                }
                break;
                case 5: double media=0; // ou double media = banco[0].getSaldo();
                for (int i=0;i<5;i++) 
                { 
                    media=media+banco[i].getSaldo();
                }
                System.out.println("\nMedia dos saldos= "+ media/5);// ou  media/banco.length
                break;
                default: break;
